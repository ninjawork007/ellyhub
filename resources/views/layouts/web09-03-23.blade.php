    ...
    <nav id="navbar-primary" class="navbar-primary desktop-only" aria-label="Navbar Primary" data-nav="flex-menu">
        <div class="container">
            <ul id="menu-navbar-primary" class="nav yamm">
                ...
                <li class="menu-item">
                    <a class="nav-link active" href="{{route('category_details',[$key->id])}}">{{ $key->name}}</a>
                    @if(count($all_sub_categories))
                    <div class="categories-mega-menus">
                        <div class="container">
                            <div class="mega-menus-inner">
                                <ul class="sub-categories">
                                    @foreach($all_sub_categories as $key1)
                                    @if($key1->category_id == $key->id)
                                    <li class="sub-category-link" >
                                        <a href="{{url('category/')}}/{{preg_replace('/[0-9\@\.\;\/]+/','-',strtolower($key->name))}}/{{preg_replace('/[0-9\@\.\;\/]+/','-',strtolower($key1->title))}}-{{ $key1->id}}">
                                            {{ $key1->title}}
                                        </a>
                                        ...
                                        <ul class="child-categories">
                                            ...
                                            <li class="\child-category-link" data-childcategory="child_category_{{$key2->id}}">
                                                <a href="{{url('category/')}}/{{preg_replace('/[0-9\@\.\;\/]+/','-',strtolower($key->name))}}/{{preg_replace('/[0-9\@\.\;\/]+/','-',strtolower($key1->title))}}/{{preg_replace('/[0-9\@\.\;\/]+/','-',strtolower($key2->name))}}-{{ $key2->id}}">
                                                    {{ $key2->name}}
                                                </a>
                                            </li>
                                            @endif
