...
$connection = curl_init();
    ...
    public function PostOnFB(){
        //1125
        //echo htmlentities ('Dell 64214-01 EqualLogic PS3000 PS4000 PS5000 PS6000 PS6010 3.5" SATA Tray'); exit;
        //$GetProduct = Product::where("upload_to_fb", 1)
        //DELETE FROM google_product_taxonomy WHERE `google_product_taxonomy`.`id` = 1125"
        // $dd = DB::table('google_product_taxonomy')
        // ->where('id', 1125)->delete();
        // echo 'delete';exit;

        $GetProduct = Product::where("status", "approved")
                ...
                <channel>
                    ...
                    foreach ($GetProduct as $GPP) {
                        ...
                        foreach ($slugParts as $part) {
                            $category = DB::table('google_product_taxonomy')
                                ->where('category_name', 'like', $part . '%')
                                ->orWhere('category_name', 'like', '%' . $part)
                                ->first();

                    
                            if ($category) {
                                $google_product_category = $category->category_name;
                                break;
                                }
                            }
                        }
                        ...
                        if (!$google_product_category) {
                            // If no matching category was found, set a default category
                        ...
                        $cleanHtmlEntFortitle = strip_tags(($GPP->name));
                        $cleanedTitle = $this->removeSpecialCharcters(mb_substr(ucwords($cleanHtmlEntFortitle), 0, 65));
                        
                        $cleanHtmlEntForDEsc = strip_tags(($GPP->description));
                        $cleanedDescription = $this->removeSpecialCharcters(mb_substr($cleanHtmlEntForDEsc,0, 9999));
			$product_real_costs = (float)($GPP->product_price) + (float)($GPP->shipping_charges);
                        ...
                        <g:availability>in stock</g:availability>
                        <g:condition>new</g:condition>
                        <g:price>' . html_entity_decode($product_real_costs) . ' USD</g:price>
    ...
    public function removeSpecialCharcters($originalString){
        $modifiedString = str_replace("*", "", $originalString);
        $modifiedString = str_replace("&", " &amp;", $modifiedString);
        //$originalStringMod = htmlentities($modifiedString);
        return ucfirst(htmlentities(strtolower($modifiedString)));
    }

    public function PostOnFBOld(){
