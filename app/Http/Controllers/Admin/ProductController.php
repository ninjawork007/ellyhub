...
class ProductController extends Controller
{
    ...
    public function product_list(Request $request)
    {
        ...
        $data  = DB::select(DB::raw("SELECT a.*,(SELECT name FROM users WHERE id=a.vendor_id) as vendor_name,(SELECT name FROM categories WHERE id=a.category_id) as category FROM products as a WHERE a.is_delete='0' AND a.status='approved' $search $where ORDER BY a.id DESC limit " . $input['start'] . "," . $input['length'] . ""));
    ...
    public function product_save(Request $request)
    {
        ...
        if ($data['user']->user_type == 'superadmin' && $request->product_state == 1) {
            $status = 'approved';
        } else {
            $status = 'pending';
        }
    ...
    public function ajax_get_product_stock(Request $request)
    {
        ...
        foreach ($data as $row) {
            ...
            $final[] = array(
                ...
                '<img src="' . $row->image . '" class="product_img">' . $row->name,
    ...
    public function ajax_get_product_bundle(Request $request)
    {
        ...
        foreach ($data as $row) {
            ...
            $final[] = array(
                ...
                '<img src="' . $row->image . '" class="product_img">' . $row->title,
    ...
    public function ajax_get_bundle_products(Request $request)
    {
        ...
        foreach ($data as $row) {
            ...
            $final[] = array(
                ...
                '<img src="' . url($row->product_image) . '" class="product_img">',
